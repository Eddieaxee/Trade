...
const FEEDS: Array<{ name: string; url: string }> = [
  ...
  { name: 'Yahoo Finance', url: 'https://feeds.finance.yahoo.com/rss/2.0/headline?s=EURUSD=X&region=US&lang=en-US' }
];
...
function parseRSS(xml: string): Array<{ title: string; link: string; date: number; summary: string }> {
  const out: Array<{ title: string; link: string; date: number; summary: string }> = [];
  const blocks = xml.match(/<(item|entry)[\s\S]*?<\/(item|entry)>/gi) ?? [];
  for (const b of blocks.slice(0, 14)) {
    const title = decode((b.match(/<title[^>]*>([\s\S]*?)<\/title>/i) ?? [])[1] ?? '');
    if (!title) continue;
    let link = (b.match(/<link[^>]*href="([^"]+)"/i) ?? [])[1] ?? '';
    if (!link) link = decode((b.match(/<link[^>]*>([\s\S]*?)<\/link>/i) ?? [])[1] ?? '');
    const dateStr = (b.match(/<(pubDate|published|updated)[^>]*>([\s\S]*?)<\//i) ?? [])[2] ?? '';
    const date = dateStr ? Math.floor(new Date(dateStr).getTime() / 1000) || 0 : 0;
    const summary = decode((b.match(/<(description|summary|content)[^>]*>([\s\S]*?)<\//i) ?? [])[2] ?? '').slice(0, 240);
    out.push({ title, link, date, summary });
  }
  return out;
}
// ── Impact lexicon: positive w → bullish for the currency it attaches to ────
interface Rule { re: RegExp; w: number; tag: string; horizon: string; impact: 'high' | 'medium' | 'low' }

const RULES: Rule[] = [
  { re: /\b(hawkish|rate hike|raises? rates|hikes?|tighten\w*|higher for longer)\b/i, w: 0.8, tag: 'policy', horizon: 'next 1–4h', impact: 'high' },
  { re: /\b(dovish|rate cut|cuts? rates|easing|stimulus|lower rates)\b/i, w: -0.8, tag: 'policy', horizon: 'next 1–4h', impact: 'high' },
  { re: /\b(cpi|inflation)\b[\s\S]{0,40}\b(hot|hotter|above|beats?|accelerat\w+|surge\w*|sticky)\b/i, w: 0.7, tag: 'inflation', horizon: 'next 15–60m', impact: 'high' },
  { re: /\b(cpi|inflation)\b[\s\S]{0,40}\b(cool\w*|below|miss\w*|softer|slows?|eases?)\b/i, w: -0.7, tag: 'inflation', horizon: 'next 15–60m', impact: 'high' },
  { re: /\b(nfp|non-?farm|payrolls?|jobs report)\b[\s\S]{0,40}\b(beat|strong\w*|above|hot)\b/i, w: 0.7, tag: 'labour', horizon: 'next 15–60m', impact: 'high' },
  { re: /\b(nfp|non-?farm|payrolls?|jobs report)\b[\s\S]{0,40}\b(miss\w*|weak\w*|below|slow\w*)\b/i, w: -0.7, tag: 'labour', horizon: 'next 15–60m', impact: 'high' },
  { re: /\b(gdp|pmi|retail sales)\b[\s\S]{0,40}\b(beat|above|strong\w*|surge\w*|expands?)\b/i, w: 0.5, tag: 'growth', horizon: 'next 1–4h', impact: 'medium' },
  { re: /\b(gdp|pmi|retail sales)\b[\s\S]{0,40}\b(miss\w*|below|weak\w*|contract\w*|slow\w*)\b/i, w: -0.5, tag: 'growth', horizon: 'next 1–4h', impact: 'medium' },
  { re: /\b(recession|crisis|default|contagion|escalat\w+|safe haven)\b/i, w: -0.6, tag: 'risk-off', horizon: 'next 4–24h', impact: 'high' },
  { re: /\b(risk-?on|optimism|rally|recovers?|rebound)\b/i, w: 0.3, tag: 'risk-on', horizon: 'next 1–4h', impact: 'low' },
  { re: /\b(intervention|jawbon\w+)\b/i, w: 0.6, tag: 'intervention', horizon: 'next 15–60m', impact: 'high' },
  { re: /\b(holds? rates|unchanged|as expected|in line)\b/i, w: 0.1, tag: 'no-change', horizon: 'next 1–4h', impact: 'low' }
];
...
function scoreHeadline(title: string, summary: string): {
  currencies: Array<{ ccy: string; delta: number }>;
  tags: string[];
  horizon: string;
  impact: 'high' | 'medium' | 'low';
  ...
  for (const r of RULES) {
    if (r.re.test(text)) {
      dir += r.w;
      tags.push(r.tag);
      if (r.impact === 'high') { impact = 'high'; horizon = r.horizon; }
      else if (r.impact === 'medium' && impact !== 'high') { impact = 'medium'; horizon = r.horizon; }
    }
  }
  ...
  return { currencies: deltas, tags, horizon, impact };
}
...
const PAIR_CCYS: Record<string, [string, string]> = {
  ...
  NZDCHF: ['NZD', 'CHF'], NZDCAD: ['NZD', 'CAD'], CADJPY: ['CAD', 'JPY'], CADCHF: ['CAD', 'CHF'], CHFJPY: ['CHF', 'JPY']
};

function actionsFromDeltas(deltas: Array<{ ccy: string; delta: number }>): NewsAction[] {
  ...
  for (const [pair, [a, b]] of Object.entries(PAIR_CCYS)) {
    ...
    out.push({ pair, bias: net > 0 ? 'long' : 'short', confidence: Math.min(95, Math.round(Math.abs(net) * 100)) });
  }
...
async function fetchFeeds(): Promise<{ items: NewsItem[]; notes: string[] }> {
  ...
  const raw: Array<{ title: string; link: string; date: number; summary: string; source: string }> = [];
  const notes: string[] = [];
  for (const s of settled) {
    if (s.status === 'fulfilled' && s.value.xml) {
      for (const it of parseRSS(s.value.xml)) raw.push({ ...it, source: s.value.name });
    } else {
  ...
  for (const r of raw) {
    ...
    if (!actions.length && Math.abs(scored.currencies[0].delta) < 0.15) continue;
    items.push({
...
async function fetchAlerts(): Promise<TechAlert[]> {
  const wanted: Array<[string, Granularity]> = [
    ['EURUSD', '15m'], ['GBPUSD', '15m'], ['USDJPY', '15m'], ['AUDUSD', '15m'],
    ['GBPJPY', '1h'], ['EURJPY', '1h']
  ];
  const watch: Array<[Pair, Granularity]> = wanted
    .map(([sym, tf]) => {
      const p = WATCHLIST.find((w) => w.symbol === sym);
      return p ? ([p, tf] as [Pair, Granularity]) : null;
    })
    .filter((x): x is [Pair, Granularity] => x !== null);
  ...
  await Promise.allSettled(
    watch.map(async ([p, tf]) => {
      try {
        ...
        const avgRange = cs.slice(-20).reduce((s, c) => s + (c.h - c.l), 0) / 20;
        if (avgRange > 0 && range > avgRange * 1.8) {
          alerts.push({
            pair: p.symbol, tf, kind: 'Momentum spike',
        ...
        if (rsiR) {
          ...
          if (Number.isFinite(v) && v >= 72) {
            alerts.push({ pair: p.symbol, tf, kind: 'RSI extreme', direction: 'down', detail: `RSI ${v.toFixed(0)} — overbought on ${tf} (${rsiR.note})`, urgency: Math.min(90, Math.round(v)) });
          } else if (Number.isFinite(v) && v <= 28) {
            alerts.push({ pair: p.symbol, tf, kind: 'RSI extreme', direction: 'up', detail: `RSI ${v.toFixed(0)} — oversold on ${tf} (${rsiR.note})`, urgency: Math.min(90, Math.round(100 - v)) });
          }
        }
      ...
      } catch { /* single pair failing must not kill the room */ }
    })
  );
