    ...
    <div className="page">
      <header className="app-header">
        <div className="brand">
          ...
          <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--muted)' }}> · SMC · CRT · confluence</span>
        </div>
        ...
        <div className="header-right">
          ...
          {snapshot && !loading && (
            <span className="pill">
              {fmtAgo(snapshot.generatedAt)} ago · {snapshot.pairs.length} pairs
              {snapshot.strength ? ` · strength ${fmtAgo(snapshot.strength.updatedAt)}` : ''}
            </span>
          )}
          {snapshot && <span className="pill">{fmtClock(snapshot.generatedAt)}</span>}
          <button
            onClick={() => void load(interval, true)}
            style={{
              ...
              <div className="cards-4">
                ...
                <div className="panel">
                  ...
                  <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 8 }}>
                    Sentiment-style analytics over historical candles. Not investment advice;
                    this dashboard never routes or executes orders.
                  </p>
                </div>
              </div>
            </div>
